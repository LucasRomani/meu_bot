...
def iniciar_cadastro_qrpedir():
    ...
    try:
        ...
        # Mapeamento (as colunas do CSV novo parecem usar os mesmos nomes)
        mapeamento = {
            "ColunaDoGrupo": "Grupo",
            ...
            "ColunaComplementoSN": "PossuiComplemento",
        ...
        # --- [NOVA LÓGICA DE AGRUPAMENTO (FORMATO COMPACTO)] ---
        log_msg("... Analisando e agrupando dados do CSV (formato compacto)...")
        itens_para_cadastrar = []
        produto_atual = None
        grupo_complemento_atual = None

        for index, row in dados_renomeados.iterrows():
            # Limpa os "identificadores de nível"
            ...
            # Nível 1: É um novo PRODUTO?
            if nome_prod:
                if produto_atual: # Salva o produto anterior
                    itens_para_cadastrar.append(produto_atual)
                
                # Cria o novo produto
                produto_atual = row.to_dict()
                produto_atual["grupos_complemento"] = []
                grupo_complemento_atual = None # Reseta o grupo
            
            # Nível 2: É um novo GRUPO DE COMPLEMENTO?
            # (Precisa de um produto_atual para pertencer)
            if produto_atual and nome_grup_comp:
            ...
            # Nível 3: É um ITEM DE COMPLEMENTO?
            if nome_item_comp:
                if grupo_complemento_atual is None:
                    # Isso pode acontecer se o produto não tiver complementos (PossuiComplemento=N)
                    # ou se o CSV estiver mal formatado.
                    if str(produto_atual.get("PossuiComplemento", "")).strip().upper() == 'S':
                        # Se era para ter complementos, é um erro
                        log_msg(f"⚠️ ERRO DE LÓGICA: Item '{nome_item_comp}' na linha {index + 2} encontrado sem um Grupo de Complemento. O item será ignorado.")
                    continue # Pula este item
                
                item_atual = row.to_dict()
                grupo_complemento_atual["itens"].append(item_atual)
        
        # Adiciona o último produto processado à lista
        if produto_atual:
            itens_para_cadastrar.append(produto_atual)
        # --- [FIM DA NOVA LÓGICA] ---
